...
func TestGetAccountTransactions(t *testing.T) {
	...
	require.Equal(t, "Equity Option", tr.InstrumentType)
	require.Equal(t, "RIVN", tr.UnderlyingSymbol)
	require.Equal(t, "Trade", tr.TransactionType)
	require.Equal(t, string(constants.STO), tr.TransactionSubType)
	require.Equal(t, "Sold 40 RIVN 06/23/23 Call 15.00 @ 0.47", tr.Description)
	require.Equal(t, string(constants.STO), tr.Action)
